...
{
    /**
     * Wire size of a `Payloads` message holding a single payload of the given data size.
     */
    private const PAYLOADS_OVERHEAD = 6;
    ...
    public function testMeasuresThePayloadsAsTheServerDoes(): void
    {
        $this->check((new StartWorkflowExecutionRequest())->setInput(self::payloads(2000)), 'StartWorkflowExecution');
    ...
    public function testDisabledLimitsSkipTheRequestEntirely(): void
    {
    ...
    private static function schedule(int $inputSize, int $memoSize): Schedule
    {
        ...
        $inputSize > 0 and $action->setInput(self::payloads($inputSize));
